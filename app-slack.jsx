...
//  DEPOT       : EmineoEducation/lumio-bc3
//  TITRE       : MSMC     BLOC : bc3
//  CLE REDIS   : msmc:bc3
...
function SlackApp({ openChannel }) {
  ...
  // F33 · L'initialisation du seed est déplacée dans l'effet de
  // restauration, plus bas : il faut d'abord savoir si une conversation
  // sauvegardée existe avant d'écraser quoi que ce soit avec le seed.
...
PLAN : ${plan.substring(0, 600)}...`;
  ...
  // On lit d'abord la session : s'il existe une conversation
  // sauvegardée, elle prime ; sinon seulement, on pose le seed.
    ...
    if (cible) {
      ...
      setTimeout(async () => {
        try {
          ...
          const history = (chatHistory[activeId] || []).filter(m => !m.typing).slice(-40).map(m => `${m.isMe ? ((window.LUMIO_DATA?._prenom)||'Consultant') : prenomCible}: ${m.text}`).join('\n');
          ...
          const resp = await window.PAC_FETCH('/api/chat', {
            method: 'POST', headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ model: 'claude-sonnet-4-6', max_tokens: 600, system: (estCommanditaire ? buildSoniaPrompt() : buildSecondairePrompt(cible.name, roleCible)) + buildDocMapBlock() + buildLivrableFactsBlock() + (window.PAC_CONV ? window.PAC_CONV() : '') + (window.__pacSessionBrief ? window.__pacSessionBrief() : ''), messages: [{ role: 'user', content: userPrompt }] })
          });
